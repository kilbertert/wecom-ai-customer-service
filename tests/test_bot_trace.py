"""BotTrace 渲染模块 + trace_extract 纯函数单测。

纯函数/类直接 import + 调用。历史 _process_bot_message_background 集成测试已在
Phase 4 重构中移除 (编排逻辑迁入 MessageProcessor, 由 tests/test_message_processor.py
覆盖; trace 三模式由 tests/test_bot_adapter.py::test_send_trace_* 覆盖)。
"""
...
from app.services.trace_extract import (
    extract_knowledge as _extract_knowledge_from_outputs,
    extract_thinking as _extract_thinking_from_outputs,
)
